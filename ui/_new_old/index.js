...
async function convertCurrency() {
    ...
    try {
        ...
        document.getElementById('result').value = `${result.result.toFixed(2)} ${to}`;
...
async function loadGraphData(data_from_conversion) {
    try {
        if (gChart) {
            gChart.destroy();
        }
        ...
        gChart = new Chart(ctx, {
            ...
            data: {
                datasets: [{
                    ...
                    data: data_from_conversion.historical_data.map(point => ({ x: point.x, y: point.y })),
                    fill: false,
